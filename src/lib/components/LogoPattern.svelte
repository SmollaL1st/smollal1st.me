	...
	function generatePattern(): LogoItem[] {
		...
		for (let ring = 0; ring < rings; ring++) {
			...
			for (let i = 0; i < itemsInRing; i++) {
				...
				const scale = 1.45 - normalizedDistance * 0.8;
				const opacity = 0.4 - normalizedDistance * 0.28;
...
<style>
	...
	.pattern-container.collapsed .logo-item {
		transform: translate(-50%, -50%) translate(0px, 0px) rotate(4deg) scale(0.3);
		opacity: 0;
